...
async function main() {
  ...
  const VSTAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 VST
  ...
  const liquidationReserve = await troveManager.VST_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(VSTAmount)

  // Total debt of the new trove = VST amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = VSTAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, VSTAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const VSTRepayment = toBN(toWei('230')) // borrower wants to repay 230 VST

  // Get trove's current debt and coll
  const { 0: debt, 1: coll } = await troveManager.getEntireDebtAndColl(borrower)

  const newDebt = debt.sub(VSTRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, VSTRepayment, false, upperHint, lowerHint, { value: collIncrease })


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(VSTAmount, price, 50)

  const { 0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedVSTAmount } = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated VST amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncatedVSTAmount,
    firstRedemptionHint,
