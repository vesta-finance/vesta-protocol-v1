...
/*
 * On deployment a new Uniswap pool will be created for the pair VST/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of VSTA token,
 ...
 * in order to avoid getting VSTA tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either VSTA token contract is deployed, and therefore VSTA tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
	...
	event VSTATokenAddressChanged(address _VSTATokenAddress);
	...
	function setParams(
		...
		_notifyRewardAmount(block.timestamp, _duration);

		emit VSTATokenAddressChanged(_VSTATokenAddress);
	...
	function claimReward() public override {
		...
		vstaToken.transfer(msg.sender, reward);
		emit RewardPaid(msg.sender, reward);
	}

	// Used only on initialization, sets the reward rate and the end time for the program
	function _notifyRewardAmount(uint256 _reward, uint256 _duration)
		internal
	{
		assert(_reward > 0);
		assert(_reward == vstaToken.balanceOf(address(this)));
