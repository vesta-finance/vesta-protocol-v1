...
contract MultiTroveGetter {
    ...
    function getMultipleSortedTroves(address _asset, int256 _startIdx, uint256 _count)
        external
        view
    ...
    function _getMultipleSortedTrovesFromHead(address _asset, uint256 _startIdx, uint256 _count)
        internal
        view
        returns (CombinedTroveData[] memory _troves)
    {
        ...
        for (uint256 idx = 0; idx < _count; ++idx) {
            ...
            (_troves[idx].debt, _troves[idx].coll,,,) = getEntireDebtAndColl(_troves[idx].asset, _troves[idx].owner);
            (_troves[idx].snapshotAsset, _troves[idx].snapshotVSTDebt) =
    ...
    function _getMultipleSortedTrovesFromTail(address _asset, uint256 _startIdx, uint256 _count)
        internal
        view
        returns (CombinedTroveData[] memory _troves)
    {
