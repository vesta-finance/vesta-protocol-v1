...
$ hardhat test
  ...
  Contract: Access Control: Liquity functions with the caller restricted to Liquity contract(s)
    ...
    ActivePool
      ...
      ✓ increaseVSTDebt(): reverts when called by an account that is not BO nor TroveM
      ✓ decreaseVSTDebt(): reverts when called by an account that is not BO nor TroveM nor SP
    ...
    DefaultPool
      ...
      ✓ increaseVSTDebt(): reverts when called by an account that is not TroveManager
      ✓ decreaseUSDV(): reverts when called by an account that is not TroveManager
    ...
    LockupContract
      ✓ withdrawVSTA(): reverts when caller is not beneficiary (68ms)
    VSTAStaking
      ✓ increaseF_VST(): reverts when caller is not TroveManager
    VSTAToken
      ✓ sendToVSTAStaking(): reverts when caller is not the VSTASstaking (49ms)
    CommunityIssuance
      ✓ sendVSTA(): reverts when caller is not the StabilityPool
      ✓ issueVSTA(): reverts when caller is not the StabilityPool
  ...
  Contract: BorrowerOperations
    Without proxy
      ...
      ✓ addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_VSTDebt snapshots (736ms)
      ...
      ✓ withdrawColl(): applies pending rewards and updates user's L_ETH, L_VSTDebt snapshots (1205ms)
      ✓ withdrawVST(): reverts when withdrawal would leave trove with ICR < MCR (516ms)
      ✓ withdrawVST(): decays a non-zero base rate (1192ms)
      ✓ withdrawVST(): reverts if max fee > 100% (668ms)
      ✓ withdrawVST(): reverts if max fee < 0.5% in Normal mode (639ms)
      ✓ withdrawVST(): reverts if fee exceeds max fee percentage (938ms)
      ✓ withdrawVST(): succeeds when fee is less than max fee percentage (1272ms)
      ✓ withdrawVST(): doesn't change base rate if it is already zero (1044ms)
      ✓ withdrawVST(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation (899ms)
      ✓ withdrawVST(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity (852ms)
      ✓ withdrawVST(): borrowing at non-zero base rate sends VST fee to VSTA staking contract (1004ms)
      ✓ withdrawVST(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (923ms)
      ✓ withdrawVST(): Borrowing at non-zero base rate increases the VSTA staking contract VST fees-per-unit-staked (986ms)
      ✓ withdrawVST(): Borrowing at non-zero base rate sends requested amount to the user (1562ms)
      ✓ withdrawVST(): Borrowing at zero base rate changes VST fees-per-unit-staked (839ms)
      ✓ withdrawVST(): Borrowing at zero base rate sends debt request to user (790ms)
      ✓ withdrawVST(): reverts when calling address does not have active trove (370ms)
      ✓ withdrawVST(): reverts when requested withdrawal amount is zero VST (459ms)
      ✓ withdrawVST(): reverts when system is in Recovery Mode (649ms)
      ✓ withdrawVST(): reverts when withdrawal would bring the trove's ICR < MCR (347ms)
      ✓ withdrawVST(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR (439ms)
      ✓ withdrawVST(): reverts if system is in Recovery Mode (312ms)
      ✓ withdrawVST(): increases the Trove's VST debt by the correct amount (208ms)
      ✓ withdrawVST(): increases VST debt in ActivePool by correct amount (230ms)
      ✓ withdrawVST(): increases user VSTToken balance by correct amount (243ms)
      ✓ withdrawVST(): reverts when repayment would leave trove with ICR < MCR (392ms)
      ✓ withdrawVST(): Succeeds when it would leave trove with net debt >= minimum net debt (435ms)
      ✓ withdrawVST(): reverts when it would leave trove with net debt < minimum net debt (256ms)
      ✓ withdrawVST(): reverts when calling address does not have active trove (431ms)
      ✓ withdrawVST(): reverts when attempted repayment is > the debt of the trove (418ms)
      ✓ withdrawVST(): reduces the Trove's VST debt by the correct amount (403ms)
      ✓ withdrawVST(): decreases VST debt in ActivePool by correct amount (406ms)
      ✓ withdrawVST(): decreases user VSTToken balance by correct amount (404ms)
      ✓ withdrawVST(): can repay debt in Recovery Mode (557ms)
      ✓ withdrawVST(): Reverts if borrower has insufficient VST balance to cover his debt repayment (1122ms)
      ...
      ✓ adjustTrove(): borrowing at non-zero base rate sends VST fee to VSTA staking contract (889ms)
      ✓ adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct (1013ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate increases the VSTA staking contract VST fees-per-unit-staked (837ms)
      ✓ adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user (895ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes VST balance of VSTA staking contract (783ms)
      ✓ adjustTrove(): Borrowing at zero base rate changes VSTA staking contract VST fees-per-unit-staked (881ms)
      ✓ adjustTrove(): Borrowing at zero base rate sends total requested VST to the user (783ms)
      ...
      ✓ adjustTrove(): reverts when VST repaid is > debt of the trove (451ms)
      ...
      ✓ adjustTrove(): changes VSTToken balance by the requested decrease (529ms)
      ✓ adjustTrove(): changes VSTToken balance by the requested increase (577ms)
      ...
      ✓ adjustTrove(): Changes the VST debt in ActivePool by requested decrease (541ms)
      ✓ adjustTrove(): Changes the VST debt in ActivePool by requested increase (397ms)
      ...
      ✓ adjustTrove(): Reverts if borrower has insufficient VST balance to cover his debt repayment (496ms)
      ...
      ✓ closeTrove(): subtracts the debt of the closed Trove from the Borrower's VSTToken balance (405ms)
      ✓ closeTrove(): applies pending rewards (1267ms)
      ✓ closeTrove(): reverts if borrower has insufficient VST balance to repay his entire debt (347ms)
      ...
      ✓ openTrove(): borrowing at non-zero base rate sends VST fee to VSTA staking contract (923ms)
      ✓ openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct (865ms)
      ✓ openTrove(): Borrowing at non-zero base rate increases the VSTA staking contract VST fees-per-unit-staked (794ms)
      ✓ openTrove(): Borrowing at non-zero base rate sends requested amount to the user (832ms)
      ✓ openTrove(): Borrowing at zero base rate changes the VSTA staking contract VST fees-per-unit-staked (602ms)
      ...
      ✓ openTrove(): records up-to-date initial snapshots of L_ETH and L_VSTDebt (639ms)
      ✓ openTrove(): allows a user to open a Trove, then close it, then re-open it (687ms)
      ✓ openTrove(): increases the Trove's VST debt by the correct amount (129ms)
      ✓ openTrove(): increases VST debt in ActivePool by the debt of the trove (162ms)
      ✓ openTrove(): increases user VSTToken balance by correct amount (85ms)
  ...
  Contract: Deployment script - Sets correct contract addresses dependencies after deployment
    ...
    ✓ Sets the correct VSTToken address in TroveManager
    ...
    ✓ Sets the correct VSTAStaking address in TroveManager
    ...
    ✓ Sets the correct VSTToken address in StabilityPool
    ...
    ✓ Sets the correct VSTAStaking address in BorrowerOperations
    ✓ Sets the correct VSTAToken address in VSTAStaking
    ✓ Sets the correct ActivePool address in VSTAStaking
    ✓ Sets the correct ActivePool address in VSTAStaking
    ✓ Sets the correct ActivePool address in VSTAStaking
    ✓ Sets the correct BorrowerOperations address in VSTAStaking
    ✓ Sets the correct CommunityIssuance address in VSTAToken
    ✓ Sets the correct VSTAStaking address in VSTAToken
    ✓ Sets the correct LockupContractFactory address in VSTAToken
    ✓ Sets the correct VSTAToken address in LockupContractFactory
    ✓ Sets the correct VSTAToken address in CommunityIssuance
...
TCR: 11.892415157517211309
  ...
  Contract: VSTA Token
    ...
    ✓ sendToVSTAStaking(): changes balances of VSTAStaking and calling account by the correct amounts (48ms)
  ...
  Contract: Deploying and funding One Year Lockup Contracts
    Deploying LCs
      ✓ VSTA Deployer can deploy LCs through the Factory (121ms)
      ✓ Anyone can deploy LCs through the Factory (71ms)
      ✓ VSTA Deployer can deploy LCs directly (43ms)
    ...
    Funding LCs
      ✓ VSTA transfer from VSTA deployer to their deployed LC increases the VSTA balance of the LC (397ms)
      ✓ VSTA Multisig can transfer VSTA to LCs deployed through the factory by anyone (168ms)
    Withdrawal attempts on funded, inactive LCs immediately after funding
      ...
      ✓ VSTA multisig can't withraw from a LC which it funded (271ms)
      ✓ No one can withraw from a LC (78ms)

  Contract: Deploying the VSTA contracts: LCF, CI, VSTAStaking, and VSTAToken
    CommunityIssuance deployment
      ✓ Stores the deployer's address
    VSTAStaking deployment
      ✓ Stores the deployer's address
    VSTAToken deployment
      ✓ Stores the multisig's address
      ✓ Stores the CommunityIssuance address
      ✓ Stores the LockupContractFactory address
      ✓ Mints the correct VSTA amount to the multisig's address: (64.66 million)
      ✓ Mints the correct VSTA amount to the CommunityIssuance contract address: 32 million
      ✓ Mints the correct VSTA amount to the bountyAddress EOA: 2 million
      ✓ Mints the correct VSTA amount to the lpRewardsAddress EOA: 1.33 million
    Community Issuance deployment
      ✓ Stores the deployer's address
      ✓ Has a supply cap of 32 million
      ✓ Liquity AG can set addresses if CI's VSTA balance is equal or greater than 32 million  (393ms)
      ✓ Liquity AG can't set addresses if CI's VSTA balance is < 32 million  (367ms)
    Connecting VSTAToken to LCF, CI and VSTAStaking
      ✓ sets the correct VSTAToken address in VSTAStaking (1866ms)
      ✓ sets the correct VSTAToken address in LockupContractFactory
      ✓ sets the correct VSTAToken address in CommunityIssuance (203ms)

  Contract: During the initial lockup period
    VSTA transfer during first year after VSTA deployment
      ✓ Liquity multisig can not transfer VSTA to a LC that was deployed directly (105ms)
      ✓ Liquity multisig can not transfer to an EOA or Liquity system contracts (302ms)
      ✓ Liquity multisig can not approve any EOA or Liquity system contract to spend their VSTA (546ms)
      ...
      ✓ Liquity multisig can not stake their VSTA in the staking contract
      ✓ Anyone (other than Liquity multisig) can transfer VSTA to LCs deployed by anyone through the Factory (359ms)
      ✓ Anyone (other than Liquity multisig) can transfer VSTA to LCs deployed by anyone directly (141ms)
      ✓ Anyone (other than liquity multisig) can transfer to an EOA (106ms)
      ✓ Anyone (other than liquity multisig) can approve any EOA or to spend their VSTA
      ...
      ✓ Anyone (other than liquity AG) can stake their VSTA in the staking contract
    Lockup Contract Factory negative tests
      ✓ deployLockupContract(): reverts when VSTA token address is not set (197ms)
    Transferring VSTA to LCs
      ✓ Liquity multisig can transfer VSTA (vesting) to lockup contracts they deployed (198ms)
      ✓ Liquity multisig can transfer VSTA to lockup contracts deployed by anyone (1798ms)
    Deploying new LCs
      ✓ VSTA Deployer can deploy LCs through the Factory (46ms)
      ✓ Liquity multisig can deploy LCs through the Factory
      ✓ Anyone can deploy LCs through the Factory (80ms)
      ✓ VSTA Deployer can deploy LCs directly (44ms)
  ...
  Contract: After the initial lockup period has passed
    Deploying new LCs
      ✓ VSTA Deployer can deploy new LCs
    ...
    Beneficiary withdrawal from initial LC
      ...
      ✓ Beneficiaries can withraw full VSTA balance of LC if it has increased since lockup period ended (234ms)
    Withdrawal attempts from LCs by non-beneficiaries
      ✓ VSTA Multisig can't withdraw from a LC they deployed through the Factory
      ✓ VSTA Multisig can't withdraw from a LC that someone else deployed
      ✓ Non-beneficiaries cannot withdraw from a LC (189ms)
    Transferring VSTA
      ✓ VSTA multisig can transfer VSTA to LCs they deployed (176ms)
      ✓ VSTA multisig can transfer tokens to LCs deployed by anyone (104ms)
      ✓ VSTA multisig can transfer VSTA directly to any externally owned account (141ms)
      ✓ Anyone can transfer VSTA to LCs deployed by anyone (179ms)
      ✓ Anyone can transfer to an EOA (421ms)
      ✓ Anyone can approve any EOA to spend their VSTA
      ...
      ✓ Anyone can stake their VSTA in the staking contract (55ms)
    Withdrawal Attempts on new LCs before unlockTime has passed
      ✓ VSTA Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, before the unlockTime (44ms)
      ✓ VSTA Deployer can't withdraw from a funded LC that someone else deployed, before the unlockTime (448ms)
    ...
    Withdrawals from new LCs after unlockTime has passed
      ✓ VSTA Deployer can't withdraw from a funded LC they deployed for another beneficiary through the Factory, after the unlockTime (100ms)
      ✓ VSTA multisig can't withdraw from a funded LC when they are not the beneficiary, after the unlockTime (529ms)
  ...
  Contract: VSTA community issuance arithmetic tests
...
issuance fraction after: 949066037374286
    ...
    ✓ Total VSTA tokens issued is 42.20 after a minute (38ms)
    ✓ Total VSTA tokens issued is 2,531.94 after an hour (41ms)
    ✓ Total VSTA tokens issued is 60,711.40 after a day (136ms)
    ✓ Total VSTA tokens issued is 422,568.60 after a week (331ms)
    ✓ Total VSTA tokens issued is 1,772,113.21 after a month
    ✓ Total VSTA tokens issued is 5,027,363.22 after 3 months (40ms)
    ✓ Total VSTA tokens issued is 9,264,902.04 after 6 months (47ms)
    ✓ Total VSTA tokens issued is 16,000,000 after a year (47ms)
    ✓ Total VSTA tokens issued is 24,000,000 after 2 years (47ms)
    ✓ Total VSTA tokens issued is 28,000,000 after 3 years (75ms)
    ✓ Total VSTA tokens issued is 30,000,000 after 4 years (40ms)
    ✓ Total VSTA tokens issued is 31,968,750 after 10 years (69ms)
    ✓ Total VSTA tokens issued is 31,999,969.48 after 20 years (68ms)
    ✓ Total VSTA tokens issued is 31,999,999.97 after 30 years (87ms)
  ...
  Contract: VSTAStaking revenue share tests
    ✓ stake(): reverts if amount is zero (74ms)
    ✓ ETH fee per VSTA staked increases when a redemption fee is triggered and totalStakes > 0 (1562ms)
    ✓ ETH fee per VSTA staked doesn't change when a redemption fee is triggered and totalStakes == 0 (3540ms)
    ✓ VST fee per VSTA staked increases when a redemption fee is triggered and totalStakes > 0 (1757ms)
    ✓ VST fee per VSTA staked doesn't change when a redemption fee is triggered and totalStakes == 0 (1705ms)
    ✓ VSTA Staking: A single staker earns all ETH and VSTA fees that occur (2143ms)
    ✓ stake(): Top-up sends out all accumulated ETH and VST gains to the staker (2138ms)
    ✓ getPendingETHGain(): Returns the staker's correct pending ETH gain (1856ms)
    ✓ getPendingVSTGain(): Returns the staker's correct pending VST gain (1965ms)
    ✓ VSTA Staking: Multiple stakers earn the correct share of all ETH and VSTA fees, based on their stake size (3133ms)
  ...
  Contract: All Liquity functions with onlyOwner modifier
    ...
    LockupContractFactory
      ✓ setVSTAAddress(): reverts when called by non-owner, with wrong address, or twice (216ms)

  Contract: StabilityPool
    ✓ getETH(): gets the recorded ETH balance
    ✓ getTotalVSTDeposits(): gets the recorded VST balance

  Contract: ActivePool
    ✓ getETH(): gets the recorded ETH balance
    ✓ getVSTDebt(): gets the recorded VST balance
    ✓ increaseUSDV(): increases the recorded VST balance by the correct amount
    ✓ decreaseUSDV(): decreases the recorded VST balance by the correct amount
    ✓ sendETH(): decreases the recorded ETH balance by the correct amount

  Contract: DefaultPool
    ✓ getETH(): gets the recorded VST balance
    ✓ getVSTDebt(): gets the recorded VST balance
    ✓ increaseUSDV(): increases the recorded VST balance by the correct amount
    ✓ decreaseUSDV(): decreases the recorded VST balance by the correct amount (57ms)
  ...
  Contract: BorrowerWrappers
    ...
    ✓ claimStakingGainsAndRecycle(): with only VST gain (821ms)
    ✓ claimStakingGainsAndRecycle(): with both ETH and VST gains (1593ms)
  ...
  Contract: StabilityPool - VSTA Rewards
    VSTA Rewards
totalVSTAIssued_1: 30370113195977152000000
totalVSTAIssued_2: 30370113195977152000000
      ✓ liquidation < 1 minute after a deposit does not change totalVSTAIssued (743ms)
      ✓ withdrawFromSP(): reward term G does not update when no VSTA is issued (634ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct VSTA gain. No liquidations. No front end. (1479ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct VSTA gain. No liquidations. No front end. (1345ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct VSTA gain. No liquidations. No front end. (1925ms)
      ✓ withdrawFromSP(): Depositor withdraws correct VSTA gain after serial pool-emptying liquidations. No front-ends. (5752ms)
      ✓ VSTA issuance for a given period is not obtainable if the SP was empty during the period (551ms)
      ✓ withdrawFromSP(): Several deposits of 100 VST span one scale factor change. Depositors withdraw correct VSTA gains (6075ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct VSTA gain. No liquidations. Front ends and kickback rates. (1660ms)
      ✓ withdrawFromSP(): Depositors with varying initial deposit withdraw correct VSTA gain. Front ends and kickback rates (2813ms)
      ✓ withdrawFromSP(): Several deposits of 10k VST span one scale factor change. Depositors withdraw correct VSTA gains (4788ms)

  Contract: Pool Manager: Sum-Product rounding errors
    - Rounding errors: 100 deposits of 100USDV into SP, then 200 liquidations of 49USDV
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ...
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing VST (965ms)
      ✓ withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing VST (1152ms)
      ...
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 VST.  A, B, C, D withdraw correct VST deposit and ETH Gain (1512ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 VST.  A, B, C, D withdraw correct VST deposit and ETH Gain (1637ms)
      ✓ withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct VST deposit and ETH Gain (1645ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 VST.  A, B, C, D withdraw correct VST deposit and ETH Gain (1704ms)
      ✓ withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct VST deposit and ETH Gain (1753ms)
      ✓ withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 VST. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct VST deposit and ETH Gain (1537ms)
...
alice deposit: 0
      ...
      ✓ withdrawFromSP(): Several deposits of 10000 VST span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (1676ms)
  ...
  Contract: StabilityPool - Withdrawal of stability deposit - Reward calculations
    Stability Pool Withdrawal
      ...
      ✓ withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing VST (1597ms)
      ✓ withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing VST (1711ms)
      ...
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 VST.  A, B, C, D withdraw correct VST deposit and ETH Gain (2168ms)
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 VST.  A, B, C, D withdraw correct VST deposit and ETH Gain (5085ms)
      ✓ withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct VST deposit and ETH Gain (2304ms)
      ✓ withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 VST.  A, B, C, D withdraw correct VST deposit and ETH Gain (2335ms)
      ✓ withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct VST deposit and ETH Gain (2145ms)
      ✓ withdrawETHGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 VST. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct VST deposit and ETH Gain (2543ms)
...
alice deposit: 0
      ...
      ✓ withdrawETHGainToTrove(): Several deposits of 10000 VST span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation (2361ms)
  ...
  Contract: StabilityPool
    Stability Pool Mechanisms
      ✓ provideToSP(): increases the Stability Pool VST balance (354ms)
      ✓ provideToSP(): updates the user's deposit record in StabilityPool (355ms)
      ✓ provideToSP(): reduces the user's VST balance by the correct amount (321ms)
      ✓ provideToSP(): increases totalVSTDeposits by correct amount (283ms)
      ...
      ✓ provideToSP(): reverts if user tries to provide more than their VST balance (1030ms)
      ✓ provideToSP(): reverts if user tries to provide 2^256-1 VST, which exceeds their balance (508ms)
      ...
      ✓ provideToSP(): providing 0 VST reverts (1091ms)
      ✓ provideToSP(), new deposit: when SP > 0, triggers VSTA reward event - increases the sum G (1072ms)
      ...
      ✓ provideToSP(), new deposit: depositor does not receive any VSTA rewards (671ms)
      ✓ provideToSP(), new deposit after past full withdrawal: depositor does not receive any VSTA rewards (1679ms)
      ✓ provideToSP(), new eligible deposit: tagged front end receives VSTA rewards (1605ms)
      ...
      ✓ provideToSP(), topup: triggers VSTA reward event - increases the sum G (1007ms)
      ✓ provideToSP(), topup from different front end: doesn't change the front end tag (1511ms)
      ✓ provideToSP(), topup: depositor receives VSTA rewards (1487ms)
      ✓ provideToSP(), topup: tagged front end receives VSTA rewards (1036ms)
      ...
      ✓ withdrawFromSP(): partial retrieval - retrieves correct VST amount and the entire ETH Gain, and updates deposit (1102ms)
      ✓ withdrawFromSP(): partial retrieval - leaves the correct amount of VST in the Stability Pool (1128ms)
      ✓ withdrawFromSP(): full retrieval - leaves the correct amount of VST in the Stability Pool (1091ms)
      ...
      ✓ withdrawFromSP(): it correctly updates the user's VST and ETH snapshots of entitled reward per unit staked (1049ms)
      ...
      ✓ withdrawFromSP(): increases depositor's VST token balance by the expected amount (2098ms)
      ...
      ✓ withdrawFromSP(): withdrawing 0 VST doesn't alter the caller's deposit or the total VST in the Stability Pool (993ms)
      ...
      ✓ withdrawFromSP(): Request to withdraw 2^256-1 VST only withdraws the caller's compounded deposit (1229ms)
      ...
      ✓ withdrawFromSP(): triggers VSTA reward event - increases the sum G (982ms)
      ✓ withdrawFromSP(), partial withdrawal: doesn't change the front end tag (1421ms)
      ✓ withdrawFromSP(), partial withdrawal: depositor receives VSTA rewards (1155ms)
      ✓ withdrawFromSP(), partial withdrawal: tagged front end receives VSTA rewards (1245ms)
      ...
      ✓ withdrawETHGainToTrove(): Applies VSTLoss to user's deposit, and redirects ETH reward to user's Trove (932ms)
      ...
      ✓ withdrawETHGainToTrove(): triggers VSTA reward event - increases the sum G (1298ms)
      ✓ withdrawETHGainToTrove(), partial withdrawal: doesn't change the front end tag (4778ms)
      ✓ withdrawETHGainToTrove(), eligible deposit: depositor receives VSTA rewards (1607ms)
      ✓ withdrawETHGainToTrove(), eligible deposit: tagged front end receives VSTA rewards (1701ms)
...
B stake after A11: 39999999999999999998008332745
  ...
  Contract: TroveManager - in Recovery Mode
    ...
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool VST > debt to liquidate: offsets the trove entirely with the pool (1265ms)
    ✓ liquidate(), with ICR% = 110 < TCR, and StabilityPool VST > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus (1382ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool VST > debt to liquidate: removes stake and updates totalStakes (1361ms)
    ✓ liquidate(), with  110% < ICR < TCR, and StabilityPool VST > debt to liquidate: updates system snapshots (1287ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool VST > debt to liquidate: closes the Trove (6532ms)
    ✓ liquidate(), with 110% < ICR < TCR, and StabilityPool VST > debt to liquidate: can liquidate troves out of order (2667ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool VST < liquidated debt: Trove remains active (985ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool VST < liquidated debt: Trove remains in TroveOwners array (1134ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool VST < liquidated debt: nothing happens (1146ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool VST < liquidated debt: updates system shapshots (1284ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool VST < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves (1266ms)
    ✓ liquidate(), with ICR > 110%, and StabilityPool VST < liquidated debt: ICR of non liquidated trove does not change (2161ms)
    ✓ liquidate() with ICR > 110%, and StabilityPool VST < liquidated debt: total liquidated coll and debt is correct (1912ms)
...
gasUsed:  636956
    ...
    ✓ batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool VST > debt to liquidate: can liquidate troves out of order (1357ms)
  ...
  Contract: TroveManager
    ...
    ✓ liquidate(): decreases ActivePool ETH and VSTDebt by correct amounts (567ms)
    ✓ liquidate(): increases DefaultPool ETH and VST debt by correct amounts (553ms)
    ...
    ✓ liquidate(): updates the L_ETH and L_VSTDebt reward-per-unit-staked totals (984ms)
    ...
    ✓ liquidate(): when SP > 0, triggers VSTA reward event - increases the sum G (1081ms)
    ...
    ✓ liquidateTroves(): when SP > 0, triggers VSTA reward event - increases the sum G (5057ms)
    ...
    ✓ batchLiquidateTroves: when SP > 0, triggers VSTA reward event - increases the sum G (1428ms)
    ...
    ✓ redeemCollateral(): cancels the provided VST with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether (838ms)
    ...
    ✓ redeemCollateral(): caller can redeem their entire VSTToken balance (1086ms)
    ✓ redeemCollateral(): reverts when requested redemption amount exceeds caller's VST token balance (1320ms)
    ✓ redeemCollateral(): value of issued ETH == face value of redeemed VST (assuming 1 VST has value of $1) (1353ms)
    ...
    ✓ redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to VSTA staking contract (5136ms)
    ✓ redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-VSTA-staked in VSTA Staking contract (1207ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to VSTA staking contract (1730ms)
    ✓ redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-VSTA-staked in the staking contract (5971ms)
    ...
    ✓ getPendingVSTDebtReward(): Returns 0 if there is no pending VSTDebt reward (618ms)
    ✓ getPendingETHReward(): Returns 0 if there is no pending ETH reward (620ms)
    ✓ computeICR(): Returns 0 if trove's coll is worth 0
    ✓ computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 VST
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 VST
    ✓ computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 VST
    ✓ computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 VST
