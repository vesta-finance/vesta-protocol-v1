 ...
 */
  ...
  it("setup: makes accounts with nominal ICRs increasing by 1% consecutively", async () => {
    ...
    const ICR_0 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[0])
    const ICR_1 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[1])
    const ICR_2 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[2])
    const ICR_3 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[3])
    const ICR_4 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[4])
    const ICR_5 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[5])
    const ICR_6 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[6])
    const ICR_7 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[7])
    const ICR_8 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[8])
    const ICR_9 = await troveManager.getNominalICR(ZERO_ADDRESS, accounts[9])
  ...
  it("getApproxHint(): returns the address of a Trove within sqrt(length) positions of the correct insert position", async () => {
    ...
    let hintAddress
      ...
      ; ({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(ZERO_ADDRESS, CR_250, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_250 = await troveManager.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const CRPercent_287 = Number(web3.utils.fromWei(CR_287, 'ether')) * 100
      ...
      ; ({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(ZERO_ADDRESS, CR_287, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_287 = await troveManager.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const CRPercent_213 = Number(web3.utils.fromWei(CR_213, 'ether')) * 100
      ...
      ; ({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(ZERO_ADDRESS, CR_213, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_213 = await troveManager.getNominalICR(ZERO_ADDRESS, hintAddress)
    ...
    const CRPercent_201 = Number(web3.utils.fromWei(CR_201, 'ether')) * 100
      ...
      ; ({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(ZERO_ADDRESS, CR_201, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_201 = await troveManager.getNominalICR(ZERO_ADDRESS, hintAddress)
  ...
  //     const hintAddress = await hintHelpers.getApproxHint(ZERO_ADDRESS,ICR, sqrtLength * 10)
  //     const ICR_hintAddress = await troveManager.getNominalICR(ZERO_ADDRESS,hintAddress)
  ...
  it("getApproxHint(): returns the head of the list if the CR is the max uint256 value", async () => {
    ...
    let hintAddress
      ...
      ; ({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(ZERO_ADDRESS, CR_Max, sqrtLength * 10, latestRandomSeed))

    const ICR_hintAddress_Max = await troveManager.getNominalICR(ZERO_ADDRESS, hintAddress)
    const ICRPercent_hintAddress_Max = Number(web3.utils.fromWei(ICR_hintAddress_Max, 'ether')) * 100

    const firstTrove = await sortedTroves.getFirst(ZERO_ADDRESS)
    const ICR_FirstTrove = await troveManager.getNominalICR(ZERO_ADDRESS, firstTrove)
  ...
  it("getApproxHint(): returns the tail of the list if the CR is lower than ICR of any Trove", async () => {
    ...
    let hintAddress
      ...
      ; ({ hintAddress, latestRandomSeed } = await hintHelpers.getApproxHint(ZERO_ADDRESS, CR_Min, sqrtLength * 10, latestRandomSeed))
    const ICR_hintAddress_Min = await troveManager.getNominalICR(ZERO_ADDRESS, hintAddress)
    const ICRPercent_hintAddress_Min = Number(web3.utils.fromWei(ICR_hintAddress_Min, 'ether')) * 100

    const lastTrove = await sortedTroves.getLast(ZERO_ADDRESS)
    const ICR_LastTrove = await troveManager.getNominalICR(ZERO_ADDRESS, lastTrove)
